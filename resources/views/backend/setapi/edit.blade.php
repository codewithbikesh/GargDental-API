              ...
              @csrf
                ...
                <div class="col-md-6">
                  ...
                  <select class="form-select" name="api_name" aria-label="Account Type">
                      ...
                      <option value="R" {{ $api->api_name == 'R' ? 'selected' : '' }}>Register</option>
                      <option value="L" {{ $api->api_name == 'L' ? 'selected' : '' }}>Login</option>
                      <option value="VE" {{ $api->api_name == 'VE' ? 'selected' : '' }}>Validate Email</option>
                      <option value="CP" {{ $api->api_name == 'CP' ? 'selected' : '' }}>Change Password</option>
                      <option value="FP" {{ $api->api_name == 'FP' ? 'selected' : '' }}>Forgot Password</option>
                      <option value="RP" {{ $api->api_name == 'RP' ? 'selected' : '' }}>Reset Password</option>
                      <option value="REVC" {{ $api->api_name == 'REVC' ? 'selected' : '' }}>Resend Email Validation Code</option>
                      <option value="UP" {{ $api->api_name == 'UP' ? 'selected' : '' }}>User Profile</option>
                      <option value="EUP" {{ $api->api_name == 'EUP' ? 'selected' : '' }}>Edit User Profile</option>
                  </select>
                </div>
