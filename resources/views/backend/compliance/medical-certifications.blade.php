    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-lg-10 d-flex flex-column align-items-center justify-content-center">
                <div class="card mb-6">
                    <div class="card-body">
                        <h5 class="card-title text-center">Add Medical Certifications</h5>

                        <form class="row g-3" method="POST" action="{{ route('backend.compliance.medical-certifications-store') }}" enctype="multipart/form-data">
                            @csrf

                            {{-- Section 1: Medical Certifications Details --}}
                            <div class="col-md-12">
                                <label class="form-label">Medical Certifications Details</label>
                                <textarea class="form-control ckeditor" name="medical_certifications" placeholder="Write description here...">{!! $medical_certifications->value ?? '' !!}</textarea>
                            </div>

                            {{-- Section 2: Multiple Additional Certifications --}}
                            <h5 class="card-title text-center">Certifications</h5>

                            <div id="certifications-wrapper">
                                <div class="certification-group row g-3">
                                    <div class="col-md-6">
                                        <label class="form-label">Certification Title</label>
                                        <input type="text" class="form-control" name="certifications[0][title]" placeholder="Enter certification title">
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label">Certificate</label>
                                        <input type="file" class="form-control" name="certifications[0][file]">
                                    </div>
                                </div>
                            </div>
                            ...
                            {{-- Certifications List --}}
                            <h5 class="card-title mt-4">Uploaded Certifications</h5>
                            <div class="row">
                              @forelse($medical_certifications->compliancefiles as $key => $file)
                                @php
                                  $fileUrl = $medical_certifications->files_full_url[$key] ?? '';
                            ...
                            <div class="text-center mt-4">
                                <button type="submit" class="btn btn-success">Submit All Certifications</button>
                            </div>
                        </form>
                    </div>
                </div>
...
<script>
    ...
    document.getElementById('add-certification').addEventListener('click', function () {
        ...
        newGroup.innerHTML = `
            <div class="col-md-6">
                <label class="form-label">Certification Title</label>
                <input type="text" class="form-control" name="certifications[${certIndex}][title]" placeholder="Enter certification title">
            </div>
            <div class="col-md-5">
                <label class="form-label">Certificate</label>
                <input type="file" class="form-control" name="certifications[${certIndex}][file]">
            </div>
